...
class FileController extends Controller
{
    ...
    public function streamFile($id, $key)
    {
        if ($id=File::whereFileableId($id)->whereKeterangan($key)->first()) {
            $file = File::find($id->id);
            if ($file->exists) {
                return response()->make($file->take, 200, [
                    'Content-Type'=>$file->mime, 'Content-Disposition'=>'inline; filename="'.$key.'.'.$file->extension.'"'
                ]);
            }
        }
        abort(404);
